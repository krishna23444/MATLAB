...
%xlabel('Time (sec.)');
%ylabel('AM signal');
subplot(2,2,1);plot(f1,xf_c,'k');grid
xlabel('Frquency (Hz)');
ylabel('Spectrum of x(t)');
axis([90000 110000 0 1]);
subplot(2,2,4);plot(f,abs(fft(mixam))/length(t),'k');grid
xlabel('Frequency (Hz)');
ylabel('Spectrum for x(n)*cos(2*pi*B*n/fs)');
subplot(2,2,3);plot(t,mixam,'k');grid
xlabel('Time (sec.)');
ylabel('x(n)*cos(2*pi*B*n/fs)');
subplot(2,2,2);plot(f,xf,'k');grid;
xlabel('Frequency (Hz)');
