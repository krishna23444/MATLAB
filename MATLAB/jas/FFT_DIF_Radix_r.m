%Radix-r Decimation in Frequency FFT.
...
%% Radix-r FFT using decimation in frequency implementation.
for a = floor((log(N)/log(r))) : -1 : 1
    for b = 1 : (N/(r^a))
        tw = 0;
        for c = (b - 1)*(r^a) + 1 : (b - 1)*(r^a) + r^(a-1)
            ...
            %Radix-r DIF using Divide and Conquer.
            z = dft_DC(z);
