...
%
% P.param1 is the window size
function imout =CM_trimmedmean(P,imin)

[m,n]= size(imin);
imout=imin;
trimsize=2;winsize=P.param1;d1=floor(winsize/2);

for i=d1+1:m-d1
  for j=d1+1:n-d1
    localim=imin(i-d1:i+d1,j-d1:j+d1);
    temp=sort(localim(:));
    imout(i,j)=mean(temp(trimsize+1:end-trimsize));
  end
end
