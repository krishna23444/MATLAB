...
for (kk = 1:length(message_pad))
    ...
    dct_block=fracF2D(cover_object(1:blocksize,1:blocksize),.6,0);
    ...
    dct_block=fracF2D(dct_block,-.6,0);
...
watermarked_image_int=double(watermarked_image);
 imwrite(watermarked_image_int,'Disfrft_watermarked.bmp','bmp');
psnr1=psnr(cover_object,watermarked_image,Nc,Mc);
psnr=10*log10(psnr1)
RMSE=sqrt((255*255)/psnr1)
BER=1/psnr
% 
% r = 100;
% alpha = 12000;
...
%  image_marked1 = imread('Disfrft_watermarked.bmp','bmp');
