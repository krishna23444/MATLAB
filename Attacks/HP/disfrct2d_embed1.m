...
%       	Block DCT Based method, using comparision between mid-band coeffcients
%           Watermark Embeding
...
k=30;           % set minimum coeff difference
...
for (kk = 1:length(message_pad))
    ...
    dct_block=Disfrct2d(cover_object(1:blocksize,1:blocksize),.4,0);
    ...
    watermarked_image(1:blocksize,1:blocksize)=Disfrct2d(dct_block,-.4,0); 
%     dct_block=Disfrct2d(dct_block,-.4,0);
...
%  imwrite(watermarked_image_int,'Disfrct_watermarked.bmp','bmp');
  imwrite(watermarked_image_int,'Disfrct_watermarked_b1.bmp','bmp');
%   [MSE,BER,PSNR] = find_ber_psnr_gray(watermarked_image,cover_object)
%   [mssim, ssim_map] = nknk1(watermarked_image,cover_object)
...
% display watermarked image
 ...
 title('Watermarked Lena Image Using Fractional Discrete Cosine Transform')
%   title('Watermarked Barbara Image Using Fractional Discrete Cosine Transform');
% MSE
% BER
% PSNR
% mssim