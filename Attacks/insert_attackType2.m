...
%
function insert_attackType2(fid,P,ATTACKCLASS)
for i=1:size(P,1)
	insert_attackType(fid,ATTACKCLASS,P(i).ref,P(i).weight,P(i).refs);
end

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function insert_attackType(fid,tag1,ref,weight,paramrefs)
begintagw(fid,tag1,ref,weight);
count=1;
%paramrefs
%paramrefs(1).label
for params=paramrefs(1).label
	fprintf(fid,'\t<param ref="%s">\n',char(params)); 
%	paramrefs(count)
	insertStructValMinMax(fid,paramrefs(count));
	endtagw(fid,'param');	
	count=count+1;
end
endtagw(fid,tag1);
