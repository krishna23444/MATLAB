...
%
function []=xml_APP_banknote(fid,P)

JPEG='J';WAVELET='W';
 fprintf(fid,'\t<description>'); 
 fprintf(fid,'Banknote Protection:  includes all attacks\n');
 fprintf(fid,'Contains attacks described by Voloshynovskiy et.al. in information Hiding Workhop 2001  \n');
 fprintf(fid,'A wide class of attacks are applicable depending on whether we want to fool\n');
  fprintf(fid,'the scanner at the input or the printer at the output\n');
  fprintf(fid,'However attacks which compress the image a lot are not applicable\n');
 fprintf(fid,'</description>\n\n');
...
[weights,values]=getweightvalues(0);
...
% for banknote protection we nly consider high to very high compression
function [weights,values]=getweightvalues(dum)
weights=[1 0.8]; 
values=['4' '3'];
