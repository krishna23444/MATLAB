...
%
% P.param1 is the angle in degrees

function imout=CM_rotationscale(P,imin)

[m,n,o]=size(imin);
im2=CM_rotation(P,imin);
if iscolor(imin)
  for i=1:3
    imout(:,:,i)=imresize(im2(:,:,i),[m n],'bicubic');
  end
else
    imout=imresize(im2,[m n],'bicubic');
end
