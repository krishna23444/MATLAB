...
la1=[1 .6228 .5531 .0768 .0011]; %MAA .52 coeffs
lb1=[.0897 .1794 0 -.1794 -.0897];
[ma1,maw1]=freqz(lb1,la1,R/2);ma1=abs(ma1);
% la1=[1 1.624 1.171 .3223 .0265]; %MAA .7 coeffs
% lb1=[.1649 .3298 0 -.3298 -.1649 ];
% [ma1,maw1]=freqz(lb1,la1,R/2);ma1=abs(ma1);
...
%% convex optimization 
v=1;
wstop = 0.72*pi;        % stopband start freq (in radians)
TOL = 1e-3; 
n = 14;                 % filter order (2n+1 is the full order)
wpass = 0.52*pi;        % passband cutoff freq (in radians)
delta = .177;              % max (+/-) passband ripple in dB
...
%% parks-mcclellan
load('D:\MATLAB 2010 Portable\bin\fipm52.mat')
...
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'-.r','LineWidth',1.3);xlabel('Frequency (Hz)');ylabel('%age error');