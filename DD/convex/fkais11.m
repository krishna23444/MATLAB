...
la1=[1 -.4398 .4672 -.0403 -.017]; %MAA .35 coefsf
...
hh=abs(H1)/pi;
 plot(w1/pi,hh,'k','LineWidth',1.7);
hold on
grid on

plot(maw1(1:5000)/pi,u(1:5000),'-.k','LineWidth',1.7)
plot(maw1/pi,abs(ma1),'r','LineWidth',1.7)
%% fourier series diff plots
 [Hf,wf] = freqz(hf,1,R/2);
ho=abs(Hf)*.1/abs(Hf(1000));
   ho=abs(Hf)/pi;
plot(wf/pi,ho,'LineWidth',1.7);

%% convex optimization 
v=1;
wstop = 0.515*pi;        % stopband start freq (in radians)
TOL = 1e-3; 
n = 24;                 % filter order (2n+1 is the full order)
wpass = 0.353*pi;        % passband cutoff freq (in radians)
delta = .1;              % max (+/-) passband ripple in dB
atten_level = -45.1;      % stopband attenuation level in dB
...
[Hfc,wfc] = freqz(hc,1,R/2);
   hoc=abs(Hfc)/pi;
plot(wf/pi,hoc,'-.b','LineWidth',1.7);
%% parks-mcclellan
% load('D:\MATLAB 2010 Portable\bin\DD\convex\fipm.mat');
% [Hfpm,wfpm] = freqz(Num,1,R/2);
%    hopm=abs(Hfpm);
% plot(wf/pi,hopm,'-.r','LineWidth',1.7);
% legend('salesnicks','alouis','convex','parks mcclellan',4)
...
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'k','LineWidth',1.7)
hold on
...
th=find(E*1000/pi>=10);
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'r','LineWidth',1.7)
grid on
...
th=find(E*1000/pi>=10);
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'LineWidth',1.7)
grid on
for i=1:R/2
    E(i)=u(i)-hoc(i);
end
th=find(E*1000/pi>=10);
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'g','LineWidth',1.7)
...
plot(maw1(1:th(1))/pi,E(1:th)*1000/pi,'-k','LineWidth',1.7);xlabel('frequency in pi units');ylabel('%age error');
legend('Salesnicks','Alaouis','Convex','Parks McClellan',4,'Location','NorthWest')
% set(2,'color','w');
set(1:2,'color','w');